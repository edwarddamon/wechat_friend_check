...
用法：
    python -m src.show_wechat           唤起微信主窗口
    python -m src.show_wechat --test    唤起 + 测 wxauto4 初始化
...
def show_wechat_window(title=WECHAT_WINDOW_TITLE, log=print):
    """从托盘/隐藏/最小化状态把微信主窗口拉出来并前置。返回是否成功。"""
    hits = find_wechat_main()
    if not hits:
        log(f"[错误] 没找到 title='{title}' 的 Qt 窗口")
        log("       请确认: 微信已登录、进程在跑")
        return False

    hwnd = hits[0]
    cls = win32gui.GetClassName(hwnd)
    log(f"[找到] hwnd={hwnd} class={cls!r} "
    ...
    log(f"[结果] visible={is_vis} is_foreground={is_fg}")
    return is_vis
...
def test_wxauto4(log=print):
    ...
    try:
        ...
        log("[错误] 没装 wxauto4，请运行: pip install wxauto4")
        return False
    ...
    try:
        ...
        try:
            ...
            log(f"  ChatInfo 调用失败(不影响主功能): {e}")
        return True
    except Exception as e:
        log(f"[失败] {type(e).__name__}: {e}")
        log("\n常见原因：")
        log("  1. 微信版本太新（wxauto4 免费版只支持到 4.1.8.107）")
        log("  2. 微信主窗口没显示出来")
        log("  3. 微信没登录")
        import traceback
        traceback.print_exc()
